WITH frame AS (
		...
		SELECT 
		np.child_protocol_id,
		array_agg(jsonb_build_object(
		'name', p.name
		,'id', np.parent_protocol_id)) AS obj
		FROM protocols.nested_protocols as np 
		LEFT JOIN protocols.protocol as p
		ON(np.parent_protocol_id = p.id)
		--WHERE depth!=0 
		--ORDER BY depth DESC
		GROUP BY child_protocol_id
),protocol AS (
		...
		SELECT 
		      ...
		      , nested_list.obj as nested_list
		      , array_agg(jsonb_build_object(
		...
		JOIN nested_list ON nested_list.child_protocol_id = p.id
		WHERE authors.login = 'testuser'
		GROUP BY p.id, authors_list.list, authors.login, authors.rank, nested_list.obj
)SELECT
	jsonb_build_object(
	...
	,'nested_list', protocol.nested_list
	) AS protocols
FROM protocol
GROUP BY protocol.protocol_id, name, frame_count, total_duration, mask_frame, protocol.authors_list, protocol.author_rank, protocol.nested_list